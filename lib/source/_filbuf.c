...
*******************************************************************************
    ...
    Module Description:
        ...
        This module contains the code for the buffer fill call.
...
int _filbuf(register FILE *stream)
{
    ...
    for (i = 0; i < FOPEN_MAX; i++) {
        if (_io[i] && io_testflag(_io[i], _IOLBF))
            if (io_testflag(_io[i], _IOWRITING))
                (void) fflush(_io[i]);
    }

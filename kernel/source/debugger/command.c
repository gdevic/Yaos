...
#include "command.h"                    // Include its own header

#include "display.h"                    // Custom print function
...
#define MAX_HISTORY         16          // Number of history buffers (hard coded)
...
static char sHistory[MAX_HISTORY][80] = // History buffer
{
...
void GetCommand( int nLine, char *sCmdLine )
{
    ...
    do
    {
        ...
        dprintf("%c%c%c%s", DP_SETCURSOR, 0, nLine, sCmd );

        // Show the cursor

        dprintf("%c%c%c%c", DP_SETLOCATTR, xCur, nLine, YELLOW COLOR_ON GREEN );
        ...
        if( xCur==79 )
            dprintf("%c%c%c ", DP_SETCURSOR, 79, nLine );
        else
            dprintf("%c%c%c%c", DP_SETCURSOR, xCur, nLine, sCmd[xCur] );

        // If a key is a combination of function keys, return the key name

        if( cKey >= F1 && cKey <= CF12)
        {
            len = strlen(sFunct[cKey-F1]);
            strcpy( &sCmd[xCur], sFunct[cKey-F1] );
            sCmd[xCur + len] = '\0';
        ...
        switch( cKey )
        {
            ...
            case HOME:
                ...
                // HOME positions the cursor at the column 0

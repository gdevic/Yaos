...
/* These macros have to get byte/word/dword from the linear memory,
   store it appropriately into the pDis->sbInstr[] buffer, advance
   bpTarget pointer by the value size and advance bInstrLen count
   by the value size
*/

#define NEXTBYTE  pDis->sbInstr[bInstrLen]=GetByte((DWORD)bpTarget++) & 0xFF; \
                  bInstrLen++

#define NEXTWORD  ((pDis->sbInstr[bInstrLen+0] = (GetByte((DWORD)bpTarget+0) & 0xFF)) << 0) +   \
                  ((pDis->sbInstr[bInstrLen+1] = (GetByte((DWORD)bpTarget+1) & 0xFF)) << 8);     \
                  bpTarget += 2; bInstrLen += 2;

#define NEXTDWORD ((pDis->sbInstr[bInstrLen+0] = (GetByte((DWORD)bpTarget+0) & 0xFF)) <<  0) +   \
                  ((pDis->sbInstr[bInstrLen+1] = (GetByte((DWORD)bpTarget+1) & 0xFF)) <<  8) +   \
                  ((pDis->sbInstr[bInstrLen+2] = (GetByte((DWORD)bpTarget+2) & 0xFF)) << 16) +   \
                  ((pDis->sbInstr[bInstrLen+3] = (GetByte((DWORD)bpTarget+3) & 0xFF)) << 24);    \
                   bpTarget += 4; bInstrLen += 4;
...
*                          disassembler.
#       (BYTE) sbInstr[16] Bytes making up the current instruction.
*
...
BYTE Dis386( TDis386 *pDis )
{
    ...
    char     *pSym;              // Symbol name
    int      dSym;               // Delta simbol name and address
...
printf("bMod:%d  bReg:%d  bRm:%d  \n", bMod, bReg, bRm );
        ...
        //
#if _TEST_
printf("bOp: %d  bDest:%d  bSrc:%d  bThird:%d  \n", bOp, bDest, bSrc, bThird );
#endif
        switch( bOp )
        {
...
printf(" arg:%02Xh ", bDest );
            ...
            switch( bDest )
            {
                 ...
                 case _Ma : // Used by bound instruction, skip the pointer info
                     ...
                     if( (bRm==4) && (bAdSize) )
                     {
                          ...
                          {
                              dwDword = NEXTDWORD;
                              bPos += sprintf( pDis->szDisasm+bPos,"[%08X", dwDword );
                          }
                          else
                          ...
                          if( bMod == 1 )
                          {
                              ...
                              if( (signed char)bByte < 0 )
                                     bPos += sprintf( pDis->szDisasm+bPos,"-%02X", 0-(signed char)bByte );
                              else
                                     bPos += sprintf( pDis->szDisasm+bPos,"+%02X", bByte );
                          }
                     ...
                     if( bMod==0 && ((bRm==6 && !bAdSize) || (bRm==5 && bAdSize)) )
                     {
                          ...
                          {
                              wWord = NEXTWORD;
                              bPos += sprintf( pDis->szDisasm+bPos,"%04X", wWord );
                          }
                          else
                          {
                              dwDword = NEXTDWORD;
                 ...
                 case _Rd : // general register double word
                     bPos += sprintf( pDis->szDisasm+bPos, "%s", sGenReg16_32[ 1 ][ bRm ] );
                 break;
                 ...
                 case _Jb : // immediate byte, relative offset
                     bByte = NEXTBYTE;
                     if( (signed char)bByte < 0 )
                         bPos += sprintf( pDis->szDisasm+bPos,"short -%02X  (%04X)",
                         0 - (signed char)bByte, bpTarget + (signed char)bByte );
                     else
                         bPos += sprintf( pDis->szDisasm+bPos,"short +%02X  (%04X)",
                         bByte, bpTarget + bByte );
                 break;
                 ...
                 case _Jv : // immediate word or dword, relative offset
                     if( bDataSize==0 )
                     {
                          ...
                          pSym = GetClosestSymbolName( &dSym, (DWORD) wWord );

                          if( pSym != NULL )
                          {
                              bPos += sprintf( pDis->szDisasm+bPos, "%s", (DWORD)bpTarget + (signed short)wWord );

                              if( dSym )
                                  bPos += sprintf( pDis->szDisasm+bPos, "%+d", dSym );
                          }
                          else
                              bPos += sprintf( pDis->szDisasm+bPos, "%+04X  (%04X)",
                                 (signed short)wWord, bpTarget + (signed short)wWord );
                     }
                     else
                     {
                          dwDword = NEXTDWORD;

                          pSym = GetClosestSymbolName( &dSym, (DWORD)bpTarget + (signed long)dwDword );

                          if( pSym != NULL )
                          {
                              bPos += sprintf( pDis->szDisasm+bPos, "%s", pSym );
                              if( dSym )
                                  bPos += sprintf( pDis->szDisasm+bPos, "%+d", dSym );
                          }
                          else
                              bPos += sprintf( pDis->szDisasm+bPos, "%+08X  (%04X)",
                                 (signed long)dwDword, bpTarget + (signed long)dwDword );
                     }
                 break;

                 case _Ob : // Simple offset
                 case _Ov :
                      // Print the segment if it is overriden
                      //
                      if( bSegOverride )
                 ...
                 case _Ap : // 32 bit or 48 bit pointer, possible segment override
                     ...
                     else
                     {
                          dwDword = NEXTDWORD;
                          wWord = NEXTWORD;
                          bPos += sprintf( pDis->szDisasm+bPos, "%s%04X:%08X",
    ...
    //
    // This code can be executed only if the instruction lenght was more than
...
DisEnd:
    ...
    if( pDis->fIllegalOp )
    {
        bPos = sprintf( pDis->szDisasm, "db     ");

        for( bOpcode=0; bOpcode<bInstrLen; bOpcode++ )
        {
            bPos += sprintf( pDis->szDisasm+bPos, "%02X", pDis->sbInstr[bOpcode] );

            if( bOpcode<bInstrLen-1 )
                bPos += sprintf( pDis->szDisasm+bPos, ", " );
        }
    }
...
int main()
{
    ...
    Dis.bDataSize = DIS_16_BIT;
    Dis.bAddressSize = DIS_16_BIT;
